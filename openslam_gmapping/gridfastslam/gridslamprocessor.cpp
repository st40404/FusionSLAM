...
(double srr, double srt, double str, double stt){
  ...
  bool GridSlamProcessor::processScan(const RangeReading & reading, int adaptParticles){
    ...
    // m_count only change when robot already moving for a while
    if (!m_count){
      m_lastPartPose=m_odoPose=relPose;
    }

    // m_odoPose is last state's relPose
    // pose using last pose (m_odoPose) and current pose (relpose) to compute the difference
